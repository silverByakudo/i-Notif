...
@IonicPage()
@Component({
  selector: 'page-app-revie',
  templateUrl: 'app-revie.html',
})
export class AppReviePage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad AppReviePage');
  }
  ...
  save() {
    let loading = this.comm.loading('Sending feedback..');
    loading.present().then(() => {
      let docId = this.fire.createId();
      this.fire.setDoc('feedBacks', docId, {
        sender: `${this.user.fname} ${this.user.lname}`,
